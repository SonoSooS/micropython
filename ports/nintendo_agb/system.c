...
const uint8_t isr_cart_header[0x100] __attribute__((section(".isr_vector"))) =
{
    ...
    /*xx,   xx,   xx,   EA*/0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
__attribuite__((noinline)) static void bare_ISR_internal(void)
{
    ...
    u32 irq_data = REG_IEIF;
    irq_data = IEIF_MERGE(irq_data);
    REG_IF = irq_data;
    //BIOS_IF |= irq_data;
    BIOS_IF = irq_data;
...
__attribute__((section(".isr_vector.start"),target("arm"),naked)) void Reset_Handler(void)
{
    ...
    BIOS_ISR_PTR = bare_ISR;
    *(unsigned volatile short*)0x04000204 = 0x4317; // WAITCNT default
